...
export default function OffersPage() {
  ...
  return (
    <div>
      <div style={s.header}><h1 style={s.title}>{t('offers.title')}</h1><button style={s.btnPrimary} onClick={() => { setShowForm(!showForm); setEditId(null); setForm({ name: '', description: '', discount_percent: 0, discount_fixed: 0, start_date: '', end_date: '', is_active: 'true' }) }}>{showForm ? `✕ ${t('common.cancel')}` : `+ ${t('offers.add')}`}</button></div>
      {showForm && (
