...
export default function SubscriptionsPage() {
  ...
  const [form, setForm] = useState({ member_id: '', plan_id: '', offer_id: '', start_date: '', end_date: '', amount_paid: 0, payment_status: 'unpaid', notes: '' })
  ...
  const filtered = useMemo(() => {
    ...
    return subs.filter(sub => {
      ...
      const searchable = [
        ...
        sub.start_date, sub.end_date, `$${sub.amount_paid}`,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await createSubscription({
        ...
        offer_id: form.offer_id ? parseInt(form.offer_id) : null, amount_paid: parseFloat(form.amount_paid)
      })
      toast(t('subscriptions.created'), 'success')
      setShowForm(false); setForm({ member_id: '', plan_id: '', offer_id: '', start_date: '', end_date: '', amount_paid: 0, payment_status: 'unpaid', notes: '' }); load()
  ...
  const handleExport = async () => {
    ...
    toast('Subscriptions exported', 'success')
  }
  ...
  const handleImport = () => {
    ...
    input.onchange = async (e) => {
      ...
      const result = await Swal.fire({
        title: `Import ${rows.length} subscriptions?`,
        html: `<div style="font-size:13px;color:#aaa">Requires valid member_id and plan_id</div>`,
        icon: 'question', showCancelButton: true, confirmButtonColor: '#2ecc71', cancelButtonColor: '#e74c3c',
        confirmButtonText: 'Import', cancelButtonText: 'Cancel', background: '#1a1a2e', color: '#e0e0e0', borderRadius: '16px',
      })
      ...
      Swal.fire({ icon: success > 0 ? 'success' : 'error', title: `${success} imported, ${fail} failed`, background: '#1a1a2e', color: '#e0e0e0', borderRadius: '16px', confirmButtonColor: '#667eea' })
      load()
    }
    input.click()
  }
    ...
    <div>
      <div style={s.header}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button style={s.btnImport} onClick={handleImport}>📥 {t('common.import') || 'Import'}</button>
          <button style={s.btnExport} onClick={handleExport}>📤 {t('common.export') || 'Export'}</button>
      ...
      <div style={s.statsRow}>
        {[
          ...
          { label: t('dashboard.totalRevenue'), value: `$${stats.revenue.toFixed(0)}`, color: '#9b59b6' },
      ...
      {showForm && (
        <form onSubmit={handleSubmit} style={s.form}>
          <div style={s.formGrid}>
            <select style={s.input} value={form.member_id} onChange={(e) => setForm({ ...form, member_id: e.target.value })} required>
              <option value="">{t('subscriptions.selectMember')}</option>
              {members.map(m => <option key={m.id} value={m.id}>{m.name} — {m.phone}</option>)}
            </select>
            <select style={s.input} value={form.plan_id} onChange={(e) => setForm({ ...form, plan_id: e.target.value })} required>
            ...
            <input style={s.input} placeholder={t('subscriptions.amount')} type="number" step="0.01" value={form.amount_paid} onChange={(e) => setForm({ ...form, amount_paid: e.target.value })} />
        ...
        <>
              ...
              return (
                <div key={sub.id} style={s.card}>
                  ...
                  <div style={s.planBadge}>
                    ...
                    <span style={s.planPrice}>${plan?.price?.toFixed(2) || '0.00'}</span>
                  </div>
                  ...
                  <div style={s.cardFooter}>
                    <div style={s.footerItem}>
                      ...
                      <span style={{ ...s.footerValue, color: '#2ecc71' }}>${sub.amount_paid?.toFixed(2)}</span>
                    </div>
                    <div style={s.footerItem}>
                      ...
                      <span style={{
                        ...
                        {sub.payment_status === 'paid' ? '✅' : sub.payment_status === 'partial' ? '⏳' : '❌'} {t(`subscriptions.${sub.payment_status}`)}
                      </span>
                    </div>
                    <div style={s.footerItem}>
          ...
          {totalPages > 1 && (
            <div style={s.pagination}>
              <button style={{ ...s.pageBtn, opacity: page <= 1 ? 0.4 : 1 }} disabled={page <= 1} onClick={() => setPage(p => Math.max(1, p - 1))}>◀ {locale === 'ar' ? 'السابق' : 'Prev'}</button>
              ...
              <button style={{ ...s.pageBtn, opacity: page >= totalPages ? 0.4 : 1 }} disabled={page >= totalPages} onClick={() => setPage(p => Math.min(totalPages, p + 1))}>{locale === 'ar' ? 'التالي' : 'Next'} ▶</button>
            </div>
          )}
        </>
      )}
...
const s = {
  ...
  card: { background: 'rgba(20,20,35,0.6)', backdropFilter: 'blur(10px)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 16, padding: 20, display: 'flex', flexDirection: 'column', gap: 14 },
